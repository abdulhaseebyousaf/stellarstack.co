...
const Testimonials = () => {
  const ExpandableText = ({ text, maxLength = 100 }) => {
    ...
    const plainText = text.replace(/<br\s*\/?\s*>/gi, " ");
    ...
    return (
      <div>
        {isExpanded || !shouldTruncate ? (
          displayedText.split(/<br\s*\/?\s*>/gi).map((line, idx, arr) => (
            <span key={idx}>
              {line}
              {idx !== arr.length - 1 && <br />}
            </span>
        ...
        {shouldTruncate && (
          <button onClick={toggleExpand} className="toggle-button text-[#204285] hover:underline focus:outline-none inline cursor-pointer ml-2">
          ...
          <div className="relative">
                ...
                <SwiperSlide key={items.id}>
                  <div className="bg-[#e4e7ee] rounded-3xl p-10 shadow-md text-gray-800 mx-4 min-h-[321px] flex flex-col items-center justify-center">
                    ...
                    <p className="italic text-xl font-normal leading-relaxed text-[#4B5563]">
                      <ExpandableText text={items.text} maxLength={315} />
                    </p>
                    <div className="mt-6 font-medium">{items.descrption}</div>
                    <div className="text-sm italic text-[#4B5563]">{items.referance}</div>
                  </div>
                </SwiperSlide>
              ))}
            </Swiper>
            <div className="custom-pagination mt-6 flex justify-center" />
            {/* Navigation buttons outside Swiper for correct placement */}
            <button
              ...
              className={`absolute left-0 inset-y-0 my-auto z-10 w-10 h-10 rounded-full flex items-center justify-center shadow-md
                ${activeIndex === 0 ? "bg-gray-300 text-gray-600 opacity-60 cursor-not-allowed" : "bg-[#204285] text-white cursor-pointer hover:bg-[#163369]"}`}
            ...
            <button
              ...
              className={`absolute right-0 inset-y-0 my-auto z-10 w-10 h-10 rounded-full flex items-center justify-center shadow-md
                ${activeIndex === Data.length - 1 ? "bg-gray-400 opacity-50 cursor-not-allowed" : "bg-[#204285] text-white cursor-pointer hover:bg-[#163369]"}`}

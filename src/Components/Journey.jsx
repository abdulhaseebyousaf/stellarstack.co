...
{Data.map((item, index) =>(
<div key={index.id} className='bg-[#CDD4DF] px-5 py-6 rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300'>
...
<img src={item.image} alt="" />
  ...
  {item.Secondhaeding}
    ...
    <p className='min-h-[72px] text-base text-[#231F20]'>{item.description}</p>
</div>
))}
</div>
        </div>

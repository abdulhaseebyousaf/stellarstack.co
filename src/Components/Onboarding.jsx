    ...
    <div className="bg-[#e3e5da] overflow-x-hidden">
      <div className="max-w-[1810px] mx-auto px-11 sm:px-16 py-28 2xl:py-36">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-[70px]">
          {Data.map((index,item) => (
            <div key={item} className="group relative rounded-[20px] lg:mr-4 ">
              ...
              <div className="relative z-10 overflow-hidden rounded-[20px] shadow-lg">
                <img
                  ...
                  alt="error"
                />
                <div className="absolute inset-0 px-4 sm:px-10 2xl:px-[105px] bg-[#204285]/60 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-center text-white text-center">
                  <h3 className="max-[460px]:text-xl text-2xl sm:text-3xl md:text-[40px] lg:text-3xl xl:text-[40px] font-bold">
              {index.heading}
                  </h3>
                  <p className="mt-2 max-[460px]:text-xs max-[530px]:text-sm text-base sm:text-xl md:text-2xl lg:text-xl xl:text-[22px] 2xl:text-[28px] font-medium leading-tight">
                    {index.description}
                  </p>
                </div>
              </div>
        ...
        <a className="flex justify-center items-center mt-20" target="_blank"  href="/about-us"><button className="flex items-center gap-2 rounded-md text-white bg-[#204285] hover:bg-[#163369] hover:shadow-lg transform hover:scale-105 transition-all duration-300 ease-in-out cursor-pointer w-max text-lg font-medium px-7 py-3"><span>Let's grow your team today!</span></button></a>
      </div>
    </div>
  );
}
